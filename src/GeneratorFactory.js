...
/* 
 * FIXME: Implement multiple renderpasses within a single function string
 */
...
/* 
 * Functions that return new transformation function 
 * New function based on existing function chain and new function passed in
 */
...
const compositionFunctions = {
  ...
  /* 
   * @function coord
   * Transforms added onto start of existing function chain
   */

  coord: existingF => newF => x => existingF(newF(x)), 

  /* 
   * @function color 
   * Transforms added onto end of existing function chain
   */

  color: existingF => newF => x => newF(existingF(x)),
...
/* 
 * @function Seq (sequence timing function)
...
const seq = (arr = []) => ({ time, bpm }) => {
  ...
  if(arr.speed){
    try {
      ...
      console.log(`Seq error: ${JSON.stringify(error, 0, 2)}`)
    }
  }
  speed = speed ? speed : 1
 ...
 * Create a function for each transform type
 */
...
/* 
 * Creates a fragment shader from object containing uniforms 
 * Creates a snippet of fragment shader code
 */
...
Generator.prototype.glsl = function (_output) {
  let output = _output || this.defaultOutput
